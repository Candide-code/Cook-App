...
- [x] **4. Message « Nouvelle recette au menu ! »** (`CODE/nouveautes/nouveautes.js`) : à l'ouverture, une boîte de dialogue annonce qu'une ou plusieurs recettes sont arrivées avec une mise à jour, sans dire lesquelles (surprise), une seule fois. La sauvegarde retient `recettesConnues`. Nouveau joueur : rien d'annoncé. Anciennes sauvegardes : les gyozas sont annoncés. (Remplace le message « nouvelle version » : inutile, le jeu charge déjà la dernière version à chaque ouverture)
